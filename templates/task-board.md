...
- **Update your row's Status whenever it changes.** When you start, change `Not started` to `In progress`. When you open a *pull request* (a request to fold your work into the main project — see the [glossary](../GLOSSARY.md)), change it to `In review`. When it's merged, change it to `Done`. The update takes about ten seconds and saves everyone a round of confusion.
- **Pick a clear branch name.** A *branch* is your own private copy of the project to work in (see the [glossary](../GLOSSARY.md)). Name it `feature/short-description` — lowercase words separated by dashes, like `feature/add-recipe-form`. Put that exact name in your row.
...
When the board is honest and current, nobody has to ask "who's doing the search feature?" or "is that done yet?" The answer is always right here.

For how tasks get chosen and split up so they don't overlap, see [Step 3: Break into tasks](../guides/step-3-break-into-tasks.md). For how to claim a task and start building on your branch, see [Step 4: Claim and build](../guides/step-4-claim-and-build.md).
...
> Going further on GitHub: This table works well while you're small. When the team outgrows it, the platform-native upgrade is to turn each row into a GitHub Issue and track them all on a GitHub Projects board, which gives you the same columns with real ownership, status, and links to the actual pull requests.
