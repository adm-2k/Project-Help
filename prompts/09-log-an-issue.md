...
- Decide if this is a **bug** (something broke) or a **feature request** (a new idea), so you pick the right template.
...
> 🎯 **Why this matters:** A vague issue ("search is weird") gets ignored. A clear one with steps to reproduce gets fixed. Good issues are how nothing important slips through the cracks.

## 🍳 RecipeRoll example

Priya notices that typing `Tomato` returns nothing, but `tomato` works. She pastes that rough note into the prompt. The AI shapes it into a bug:
...
> - **No steps to reproduce, no fix.** For bugs, the numbered steps are the most important part — make sure they actually trigger the problem.
> - One issue per note. If your rough note describes two problems, ask the AI to split it into two issues.
> - Pick the right template: bugs and feature requests have different shapes.
