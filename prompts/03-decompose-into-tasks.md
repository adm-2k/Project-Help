...
A *branch* is your own private copy of the project, where you build your feature without disturbing anyone else's. A *branch name* like `feature/add-recipe-form` just labels that copy. See the [glossary](../GLOSSARY.md) and [How git works](../HOW-GIT-WORKS-FOR-NON-CODERS.md).
...
- Pay close attention to "Collision risks." Those are the spots where merge conflicts could happen, and a little planning now saves headaches later.
...
Collision risk flagged: Tasks B and C both touch the list page. The fix is to let Sam finish the basic list page first, then have Priya add search on top, so they don't edit the same file at once.

> Watch out: Don't skip the collision check. Two people editing the same file is the most common cause of a [merge conflict](../GLOSSARY.md), and a little sequencing avoids it.
