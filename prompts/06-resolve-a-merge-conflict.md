...
First, breathe. A [merge conflict](../GLOSSARY.md) is **normal and easy to fix**. It just means you and a teammate edited the same lines of the same file, and git politely wants a human to decide which version to keep. Nothing is broken, and nothing is lost. For the full mental picture, see [How git works for non-coders](../HOW-GIT-WORKS-FOR-NON-CODERS.md).
...
- ✅ Note which file (or files) is showing a conflict. Your AI builder or GitHub will name them for you — for example "Conflict in recipes.js".
- ✅ Don't manually delete the strange `<<<<<<<`, `=======`, `>>>>>>>` markers yourself. Let the AI explain them first.

> 💡 **Why this matters:** Conflicts feel scary the first time because the file looks messy. But the fix is just a conversation: the AI explains both versions in plain English, you pick what's right, and it cleans up. Most conflicts are resolved in a minute or two.
...
Priya says yes. The AI keeps both functions, deletes the `<<<<<<<` markers, saves the file, and tells her the merge is now complete. Total time: about a minute. (She also notes a real bug to log later: search is case-sensitive, so "Tomato" misses "tomato".)

> ⚠️ **Watch out**
> - Don't hand-edit the conflict markers (`<<<<<<<`, `=======`, `>>>>>>>`) yourself before the AI explains them — it's easy to delete the wrong half. Let the AI guide you.
> - "Keep both" is often the right answer when the two changes are unrelated. "Keep one" is right when they're two versions of the same thing. Ask the AI which case you're in.
