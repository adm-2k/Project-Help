...
A _pull request_ (PR) is you saying "here's my finished piece, please check it and add it to the shared version." A small, tested, well-described PR gets reviewed in minutes and keeps `main` working. A messy one creates confusion and slows everyone down. This list gets yours into good shape.

> Tip: A pull request is not scary; it's a polite request to merge your branch. See the [glossary](../GLOSSARY.md) if the words feel new.
...
> Watch out: Resist the urge to "just fix one more thing" while you're here. Extra, unrelated changes make a PR slow and risky to review. Keep it to your one task.
