...
Right now your plan is just words on a page, so changing it is free. Once people start building, changing the plan means throwing away work. The cheapest moment to fix a confusing or oversized plan is this meeting, before a single line of the app exists. And remember: if a teammate reads the plan and feels confused, your [AI builder](../GLOSSARY.md) (Claude Code, Cursor, etc.) will be confused too.
...
3. **Let the AI play skeptical product manager.** Paste the prompt from [../prompts/02-stress-test-prd.md](../prompts/02-stress-test-prd.md) into your AI builder, along with your PRD. It will act as a tough but fair reviewer and surface:
...
5. **Resolve each issue.** For every flag, either tighten the wording or cut the feature out of v1. Do not leave anything unresolved.

6. **Record your choices.** Every real decision goes into [../templates/DECISIONS.md](../templates/DECISIONS.md), in one short line, so nobody re-argues it later.
...
> 💡 **Tip:** Define every fuzzy word. "Fast", "simple", and "easy" mean nothing until you attach a number or an example. "Fast" becomes "the recipe list loads in under 2 seconds".

## 🍳 RecipeRoll example

The team reads the RecipeRoll PRD aloud. When they hit the "Add a recipe" section, Sam asks a sharp question:

> 💬 "What happens if two people add a recipe with the same name? Do we block it? Warn them? Allow it?"

Nobody had thought about it. The AI's stress-test had also flagged it as an open question. The team talks for two minutes and decides: **duplicate recipe names are allowed for v1 — keep it simple; revisit if it becomes confusing.**
...
They also catch a vague phrase: "search should be fast". They replace it with a real success criterion: **"A new visitor can add a recipe and then find it by searching one of its ingredients, in under a minute, without help."** Then they confirm photos, ratings, logins, and comments are all **Later**, and lock v1.
...
> - **Leaving fuzzy words undefined.** "Simple", "fast", "intuitive", "clean" are traps. Replace each one with a number, an example, or a concrete sentence.
> - **Letting scope quietly grow.** Every "we could also..." is a Later, not a v1. Write it down under Later and move on.
