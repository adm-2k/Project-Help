...
> In this step you propose adding your finished piece to the shared project, so a teammate can review it before it becomes part of the real app.

**Goal:** open a clear, well-described pull request from your branch. **Time:** about 15 minutes. **Who:** the person who built the task.

## 💡 Why this matters

A [pull request](../GLOSSARY.md) (PR) is simply a polite request that says: "Here is my piece. Please review it and, if it looks good, add it to the shared project." It is the moment your work goes from "just mine" to "let's check this together." A good PR makes your teammate's review fast and friendly, and it leaves a record of what changed and why. Nothing merges into the shared project without one, so this is the safety gate that keeps the team's app stable.

> 💬 **Why this matters:** Think of a PR like handing in a short, tidy report instead of a messy pile of pages. The clearer your hand-off, the faster and kinder the review.

## ✅ What you'll do

First, make sure you've already run the checks in [../checklists/before-you-open-a-pr.md](../checklists/before-you-open-a-pr.md). Open a PR only when your piece actually works.

1. **Push your branch.** "Pushing" means uploading the work from your computer to the shared project online (on [GitHub](../GLOSSARY.md)). In your AI builder (Claude Code, Cursor, etc.) or in GitHub Desktop, click the **Push** button. (Optional command-line equivalent, for reference only:)

   ```
   git push
   ```

2. **Open a pull request from your branch into `main`.** [`main`](../GLOSSARY.md) is the shared, trusted version of the app. On GitHub.com, after pushing, you'll usually see a yellow banner with a **"Compare & pull request"** button — click it. (Your AI builder and GitHub Desktop also have a "Create Pull Request" button.) Confirm it is merging **from** your branch **into** `main`.
...
5. **Link the related task or issue.** Connect this PR to the task you claimed on the [task board](../templates/task-board.md). Typing `Closes #12` (using your task's number) in the description tells GitHub to tick that task off automatically when the PR is merged.

6. **Request a teammate as reviewer.** On the right side of the PR page, click **Reviewers** and pick one teammate. Tell them in your team chat that it's ready.

> 💡 **Tip:** Keep PRs small and focused — one feature or fix per PR. A small PR is reviewed in minutes; a giant one sits for days.
...
She links it to her task (`Closes #3`) and requests **Sam** as her reviewer. Done — it's now waiting for a friendly second pair of eyes.
...
> - **Not testing first.** Never open a PR for something you haven't actually run. Test it, then share it.
